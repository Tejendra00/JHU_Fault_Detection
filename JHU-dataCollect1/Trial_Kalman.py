...
class KalmanFilter(object):
    def __init__(self, F = None, B = None, H = None, Q = None, R = None, P = None, x0 = None,U=None):
    ...
    def update(self,y,H,R):
        self.IM=np.dot(H,self.x)
        self.IS = R + np.dot(H, np.dot(self.P, H.T))
        self.K = np.dot(self.P, np.dot(H.T, inv(self.IS)))
        self.x = self.x - np.dot(self.K, (y-self.IM))
...
def example(data):
    dt=1/30
    F= np.array([[1, dt], [-dt, 1]])
    H = np.array([[1,0], [0,1]])
    Q = np.array([[0.01, 0.01], [0.01, 0.01]])
    R = np.array([[5,0],[0,3]]).reshape(2, 2)
    ...
    a=data
    Measurements=[0]*len(a)
    i=0
    for z in a:
        Measurements[i]=np.array([[z],[0]])
        i+=1
        
        
    # kf = KalmanFilter(F = F, B=B, H = H, Q = Q, R = R,P=P,x0=Measurements[0],U=U)
    predictions = []
    kf = KalmanFilter(F = F, B=B, H = H, Q = Q, R = R,P=P,U=U)
    # N_iter=50
