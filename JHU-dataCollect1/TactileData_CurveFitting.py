...
class CurveFit():
    def __init__(self):
        self.filename="C:\Users\preet\OneDrive\Documents\JHU\JHU-dataCollect\10.txt"
        self.timme="10.csv"
        self.numSensors = 16
    ...
    def _stackPlot(self,):
        ...
        for i in range(16):
            axis[i//4, i%4].scatter(range(len(self.data_dict[str(i+1)])),self.data_dict[str(i+1)])
            axis[i//4, i%4].plot(self.fit_dict[str(i+1)])
    ...
    def fit(self,):
        ...
        for i in range(self.numSensors):
            ...
            self.fit_dict[str(i+1)] = test(x,param[0],param[1])
